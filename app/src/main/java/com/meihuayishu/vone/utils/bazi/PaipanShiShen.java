...
public class PaipanShiShen {
    ...
    private String paipan(Calendar cal, Lvhehun.sex isman) throws ParseException {
        ...
        for (int i = 0; i < DayunArray.length; i++) {
            DayunArrayshengsi[i] = shengwang[0][mPaipandayun.getyuezhuStart(shengwang[shengsibiaoshunxu], DayunArray[i].substring(1, 2))]; //十天干生旺死绝表 用干查表

            DayunArrayshisheng[i] = shisheng[shishengbiaoshunxu][mPaipandayun.getyuezhuStart(shisheng[0], DayunArray[i].substring(0, 1))]; //十神表 用支查表
        }
