...
//        Log.i("MSL", "requestPermission: " + indexs.size());
        if (indexs.size() == 0) {
        } else {
...
//            Log.e("MSL", "requestPermission: " + unCompletePermissionList.size());

            final String[] unCompletePermissions = unCompletePermissionList.toArray(new String[unCompletePermissionList.size()]);
//            Log.i("MSL", "requestPermission: " + unCompletePermissions.toString());
    ...
    private static void showMessageOKCancel(Activity activity, String message, DialogInterface.OnClickListener okListener) {
        new AlertDialog.Builder(activity)
                .setMessage(message)
                .setPositiveButton("朕知道了", okListener)
